  ...
  return (
    <section className="space-y-10">
      <div className="section-heading">
        <div>
          <h2 className="section-title">{t('Gestor de Test Cases')}</h2>
          <p className="text-sm text-slate-300">
            {t('Planifica, ejecuta y monitorea los casos alineados a cada historia de usuario con seguimiento visual en tiempo real.')}
          </p>
        </div>
        <div className="flex flex-wrap gap-3 text-xs uppercase tracking-[0.25em] text-slate-300">
          <span className="rounded-2xl border border-white/10 px-3 py-2">{testCases.length} {t('casos')}</span>
          <span className="rounded-2xl border border-white/10 px-3 py-2">{userStories.length} {t('historias')}</span>
        </div>
      </div>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-white">{t('Filtros rápidos')}</h3>
            <p className="text-sm text-slate-300">{t('Refina por estado, sprint o historia de usuario.')}</p>
          </div>
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            >
              <option value="all">{t('Estado · Todos')}</option>
            ...
            <select
              ...
              className="rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white focus:border-neon/40 focus:outline-none"
            >
              <option value="all">{t('Sprint · Todos')}</option>
              {sprints.map((sprint) => (
            ...
            <select
              ...
              className="rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white focus:border-neon/40 focus:outline-none"
            >
              <option value="all">{t('Historia · Todas')}</option>
          ...
          <table className="min-w-full divide-y divide-white/5 text-sm">
            <thead className="bg-white/5 uppercase tracking-[0.2em] text-xs text-slate-400">
              <tr>
                ...
                <th className="px-4 py-3 text-left">{t('Descripción')}</th>
                <th className="px-4 py-3 text-left">{t('Historia')}</th>
                <th className="px-4 py-3 text-left">{t('Sprint')}</th>
              ...
              {filteredCases.map((testCase) => (
                <tr key={testCase.id} className="transition hover:bg-white/10">
                  ...
                  <td className="px-4 py-3">
                    <StatusPill label={t(testCase.status)} variant={statusVariants[testCase.status] ?? 'neutral'} />
                  </td>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel lg:col-span-2">
          <header className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white">{t('Detalle de pasos')}</h3>
              <p className="text-sm text-slate-300">{t('Selecciona un caso para visualizar pasos y resultado esperado.')}</p>
            </div>
          </header>
            ...
            {filteredCases.slice(0, 4).map((testCase) => (
              <div key={testCase.id} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                ...
                <p className="mt-4 text-xs text-slate-400">
                  {t('Resultado esperado')}: <span className="text-slate-200">{testCase.expectedResult}</span>
                </p>
              </div>
            ))}
          </div>
        ...
        <motion.aside initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel">
          <h3 className="text-lg font-semibold text-white">{t('Crear nuevo test case')}</h3>
          <p className="mt-2 text-sm text-slate-300">{t('Completa los campos esenciales y vincula una historia de usuario.')}</p>
          <form className="mt-4 space-y-4" onSubmit={handleCreate}>
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Título')}</label>
              <input
                value={form.title}
                ...
                placeholder={t('Validar login con MFA')}
              />
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Descripción')}</label>
              <textarea
                ...
                placeholder={t('Detalle breve del objetivo del caso')}
              />
            </div>
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Historia')}</label>
              <select
              ...
              >
                <option value="">{t('Selecciona una historia')}</option>
                {userStories.map((story) => (
                  <option key={story.id} value={story.id}>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Sprint')}</label>
              <input
                value={form.sprint}
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('Resultado esperado')}</label>
              <input
                value={form.expectedResult}
                onChange={(event) => setForm((prev) => ({ ...prev, expectedResult: event.target.value }))}
                className="w-full rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-neon/40 focus:outline-none"
                placeholder={t('El usuario accede al dashboard')}
              />
            </div>
            <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cobalt via-magenta to-neon px-4 py-3 text-sm font-semibold text-white shadow-glow transition hover:shadow-lg">
              <PlayIcon className="h-4 w-4" /> {t('Guardar y ejecutar')}
            </button>
          </form>
        </motion.aside>
      </div>
