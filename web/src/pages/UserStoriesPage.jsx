  ...
  return (
    <section className="space-y-10">
      <div className="section-heading">
        <div>
          <h2 className="section-title">Backlog y User Stories</h2>
        ...
        <div className="flex flex-wrap gap-3 text-xs uppercase tracking-[0.25em] text-slate-300">
          <span className="rounded-2xl border border-white/10 px-3 py-2">{stats.total} historias</span>
          <span className="rounded-2xl border border-white/10 px-3 py-2">{stats.linked} vínculos</span>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel lg:col-span-2">
          <header className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-white">Historias activas</h3>
              <p className="text-sm text-slate-300">Prioriza por impacto y cubre criterios antes del cierre de sprint.</p>
            </div>
          </header>
            ...
            {userStories.map((story) => (
              <article key={story.id} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <StatusPill label={story.status} variant={statusVariants[story.status] ?? 'info'} />
                </div>
                <p className="text-xs text-slate-300">{story.description}</p>
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Criterios</p>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-slate-300">
                  Test cases vinculados: {story.linkedTestCases.length ? story.linkedTestCases.join(', ') : 'Ninguno'}
                </div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500">Owner · {story.owner}</p>
              </article>
            ))}
          </div>
        </motion.section>
        ...
        <div className="space-y-6">
          <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel">
            <h3 className="text-lg font-semibold text-white">Crear historia</h3>
            <p className="mt-2 text-sm text-slate-300">Define título, prioridad y criterios clave.</p>
            <form className="mt-4 space-y-3" onSubmit={handleCreate}>
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Estado</label>
                <select
                  value={form.status}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Criterios de aceptación</label>
                <textarea
                  ...
                  placeholder="Cada criterio en una línea"
                />
              </div>
          ...
          <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel">
            <h3 className="text-lg font-semibold text-white">Vincular test case</h3>
            <p className="mt-2 text-sm text-slate-300">Relaciona un caso existente para asegurar cobertura.</p>
            <form className="mt-4 space-y-3" onSubmit={handleLink}>
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Historia</label>
                <select
                  value={selectedStory}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Test case</label>
                <select
