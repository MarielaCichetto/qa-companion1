...
const STATUS_COLUMNS = [
  { key: 'Abierto', titleKey: 'To Do', accent: 'cobalt' },
  { key: 'Investigando', titleKey: 'Investigating', accent: 'magenta' },
  { key: 'En progreso', titleKey: 'In Progress', accent: 'neon' },
  { key: 'Resuelto', titleKey: 'Done', accent: 'lime' }
];
  ...
  return (
    <section className="space-y-10">
      <div className="section-heading">
        <div>
          <h2 className="section-title">{t('Incidencias estilo Jira')}</h2>
          <p className="text-sm text-slate-300">
            {t('Coordina la resolución de bugs con vista kanban, prioridades visibles y métricas de severidad por proyecto.')}
          </p>
        </div>
        <div className="flex items-center gap-3 text-xs uppercase tracking-[0.25em] text-slate-300">
          <span className="rounded-2xl border border-white/10 px-3 py-2">
            {bugs.length} {t('bugs')}
          </span>
        </div>
      </div>
        ...
        {STATUS_COLUMNS.map((column, columnIndex) => (
          ...
          >
            <header className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-white">{t(column.titleKey)}</h3>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-400">
                  {groupedBugs[column.key]?.length ?? 0} {t('items')}
                </p>
              </div>
            ...
            <div className="space-y-4">
              {groupedBugs[column.key]?.map((bug) => (
                <article key={bug.id} className="space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4 shadow-inner">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <StatusPill label={t(bug.severity)} variant={severityColor[bug.severity] ?? 'info'} />
                  </div>
                  <p className="text-xs text-slate-300">
                    {t('Reportado por')} {bug.reporter} · {t('Asignado a')} {bug.assignee}
                  </p>
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-400">
                    ...
                    >
                      {column.key === 'Resuelto' ? t('Listo') : t('Mover')}
                    </button>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <ChatBubbleBottomCenterTextIcon className="h-4 w-4" /> {t('Notas')}
                    </button>
                    {selectedBug === bug.id && (
                      <div className="space-y-2">
                        <textarea
                          ...
                          placeholder={t('Agregar comentario de seguimiento')}
                        />
                        <button
                          type="button"
              ...
              {!groupedBugs[column.key]?.length && (
                <p className="rounded-2xl border border-white/10 bg-white/5 p-4 text-center text-xs text-slate-500">
                  {t('Sin registros')}
                </p>
              )}
            </div>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel">
        <header className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">{t('Registrar nuevo bug')}</h3>
            <p className="text-sm text-slate-300">{t('Documenta severidad, prioridad y contexto clave.')}</p>
          </div>
        </header>
        <form className="mt-6 grid gap-4 md:grid-cols-2" onSubmit={handleCreateBug}>
          <div className="space-y-2 md:col-span-2">
            ...
            <input
              ...
              placeholder={t('Error 500 al exportar CSV')}
            />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">{t('Severidad')}</label>
            <select
              value={form.severity}
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white focus:border-neon/40 focus:outline-none"
            >
              <option value="Crítica">{t('Crítica')}</option>
              <option value="Alta">{t('Alta')}</option>
              <option value="Media">{t('Media')}</option>
            </select>
          </div>
          <div className="space-y-2">
            <label className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">{t('Proyecto')}</label>
            <input
              value={form.project}
              onChange={(event) => setForm((prev) => ({ ...prev, project: event.target.value }))}
              className="w-full rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-neon/40 focus:outline-none"
              placeholder={t('QA Companion')}
            />
          </div>
          <div className="space-y-2 md:col-span-2">
            ...
            <textarea
              ...
              placeholder={t('Detalle de impacto y resultado esperado')}
            />
          </div>
          <button
            type="submit"
